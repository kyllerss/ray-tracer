...
pub fn run() -> Result<(), Error> {
    ...
    // sphere object
    let in_front_of_wall_translation = Matrix::new_translation(0.0, 0.0, 5.0);
    let sphere = Sphere::new(in_front_of_wall_translation);

    let canvas_length: usize = 100;
    let center_point = (canvas_length as f64 / 2.0).round();
    ...
    for row in 0..canvas_length {
        for col in 0..canvas_length {
            // point to cast a ray to is
            let x_wall_point = col as f64 - center_point;
            let y_wall_point = row as f64 - center_point;
            ...
            if !intersections.is_empty() {
                ...
                canvas.render(wall_point, Color::new(1.0, 0.0, 0.0));
            }
