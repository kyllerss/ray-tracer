...
fn ch7_test2_validate_default_world() {
    ...
    let s1_exp: Arc<dyn Renderable> = Arc::new(Sphere::new_material(m1));
    assert!(w.objects.contains(&s1_exp));

    let t2 = Matrix::new_scaling(0.5, 0.5, 0.5);
    let s2_exp: Arc<dyn Renderable> = Arc::new(Sphere::new(t2));
...
fn ch7_test6_shading_an_intersection_inside_and_outside() {
    ...
    let i = Intersection::new(4.0, shape.deref());
    ...
    let i = Intersection::new(0.5, shape.deref());
    let comps = Computations::prepare_computations(&i, &r);
...
fn ch7_test9_color_intersection_behind_ray() {
    let mut w = build_test_world();
    w.objects[0].deref().shape().material.ambient = 1.0;
    w.objects[1].deref().shape().material.ambient = 1.0;
    let r = Ray::new(Point::new(0.0, 0.0, 0.75), Vector::new(0.0, 0.0, -1.0));
    let c = w.color_at(&r);
    let c_exp = w.objects[1].deref().shape().material.color;
    assert_eq!(c, c_exp);
}
...
fn ch8_test6_shade_hit_is_given_intersection_in_shadow() {
    ...
    let s1 = Arc::new(Sphere::new_unit());
    w.objects.push(s1);
    let s2 = Sphere::new(Matrix::new_translation(0.0, 0.0, 10.0));
    w.objects.push(Arc::new(s2.clone()));
    let r = Ray::new(Point::new(0.0, 0.0, 5.0), Vector::new(0.0, 0.0, 1.0));
